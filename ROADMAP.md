...
- **A troca de modelo no posto é inalcançável em jogo** (visto em 2026-09-17, decisão: deixar assim por ora). O modo TROCAR só lista modelos com o time ativo **vazio** (`RelicStationScreen._swap_rows`, seguindo o "esvaziar os slots antes" do documento `relicario`), mas `PlayerRoster.deposit` recusa deixar zero ativa — o jogador sempre precisa de alguém para andar no mapa. As duas regras estão certas sozinhas e se anulam juntas. Sem sistema de aquisição não há o que trocar, então não dói ainda; quando a aquisição entrar, a regra da troca precisa ser redesenhada junto — as opções medidas: permitir a troca quando o time ativo cabe no `slotCapacity` do modelo novo (mantém sempre uma ativa, exige depositar só o excedente), ou permitir zero ativa só dentro do posto e exigir retirar uma antes de sair.
...
O que **fechou** aqui em 2026-09-17: a progressão ficou visível (barra, XP faltante, material e quantidade em bolsa na janela do time, no painel da ativa, no set e no posto; mensagem pós-luta com a quantidade), o posto paginou o storage e passou a mostrar nível/HP/XP nas duas listas, o starter neutro deixou de pedir um material que não existe, e a barra de captura do relicário ganhou teste. Ver README, "Nível e XP" e "Relicário".
...
### 8. Vila da costa — o que ficou aberto depois da mini vila (2026-09-20)

Fechado nesta rodada: três estruturas afastadas (`VILLAGE_SPACING` 9 m) e recuadas 8 m da rampa, uma pessoa do kit diante de cada uma (placa e clique sobre ela), praça pavimentada no shader (`village_mask_tex`) com lajes do MegaKit na orla. Ver README, "Pontos fixos". O que sobrou:

- **A cerca de pedra e corda** (`models/village/props/cerca_corda.glb`) ainda não existe — as três entradas em `MapDressing.PZ01_VILLAGE_PROPS` são puladas até o arquivo chegar (gerado no Tripo Studio, copiado à mão; ver o comentário da tabela).
- **O jogador atravessa as estruturas.** O corpo de clique de cada serviço continua sendo a cápsula/caixa antiga no centro do prédio; uma caixa do tamanho do `.glb` bloquearia a passagem e precisaria de camadas de colisão pensadas (spawn e pessoa ficam fora dela: NPC a `fundo/2 + 0,8 m`, spawn a 4,5 m).
- **A duelista da arena** (NPC-002, receita no bundle) ainda é só a estrutura `arena.glb` — decisão de 2026-09-20 de deixar a ilha como está; `attach_npc(appearance, +1.0, "Sword_Idle")` é a linha que falta, e `Sword_Idle` precisaria entrar em `LOOPED_CLIPS`.
- **Receitas fixas de posto e bancada** (`NPC_RECIPE`) migram para `npc_appearances` quando os dois virarem NPC de catálogo (`npc_role` já tem `flavor`; falta o export emitir e o `BestiaryData` ler).
- **A vila inteira não cabe numa tela** (~23,7 m contra ~24 m de largura da câmera). Se incomodar, `VILLAGE_SPACING` 8 m ainda não sobrepõe (vão de 2,3 m).
...
**Os dois parágrafos abaixo (pipeline Meshy) são histórico — desatualizados desde a migração pro fluxo "base + casca" da mestre (2026-09-15/17, ver `avyron/CLAUDE.md`, "Corpo de criatura é 'base + casca'").** O elenco do PZ-01 (14 espécies) já fechou 100% por esse caminho novo, não pelo Meshy descrito abaixo. O que falta hoje não é mais "corpo definitivo vs. placeholder" — é a ANIMAÇÃO em si: todo o elenco (incluindo o PZ-01 já completo) toca clipes emprestados de um pack humano (golpe de espada/escudo/magia), retargetados na UAL compartilhada. Ver `ANIMACOES_PENDENTES.md` pro estado exato e a decisão em aberto (clipe novo compartilhado vs. clipe por criatura).
